 ...
 *
 * @param array<string|int|array<string>> $company_data Company data
 * @param array<string>                   $params       Array of params:
 *                                                          - 'field_prefix' - prefix for company profile fields
 *
 * @return array<string|int|array<string>> $company_data
 */
function fn_filter_company_data_by_profile_fields(array $company_data = [], array $params = [])
{
    ...
    array_walk(
        ...
        static function ($value, $field_name) use (&$company_data, $params) {
            if (
                !(
                    isset($company_data[$params['field_prefix'] . $field_name])
                    && !YesNo::toBool($value['storefront_show'])
                )
            ) {
                return;
            }
