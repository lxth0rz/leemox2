    ...
    if ($result->isSuccess()) {
        ...
        if ($message_id) {
            ...
            if ($notify_by_email && Registry::get('settings.Appearance.email_templates') == 'new') {
                ...
                } elseif ($thread_full_data['communication_type'] == CommunicationTypes::VENDOR_TO_CUSTOMER) {
                    $force_notification['A'] = false;
                    $force_notification[$message_data['user_type']] = false;
                }

                if (fn_allowed_for('MULTIVENDOR') && $thread_full_data['communication_type'] == CommunicationTypes::VENDOR_TO_ADMIN) {
                    $force_notification['C'] = false;
                    $force_notification[$message_data['user_type']] = false;
                }
