...
class Manager
{
    ...
    public function add(array $data)
    {
        ...
        $this->updateStatistics(['preset_id' => $id]);
        $data['preset_id'] = $id;
    ...
    public function clonePreset($preset_id, $company_id)
    {
        ...
        if (!$company_id || $company_id === $preset['company_id']) {
            if (!empty($preset['file'])) {
                $this->file_manager->copyUpload($preset['file'], $preset['company_id']);
            }
        } else {
            unset($preset['last_launch'], $preset['last_status'], $preset['last_result'], $preset['file']);
        }
    ...
    public function find(
        ...
        array $condition = [],
        $join = null,
        array $fields = ['ip.*', 'ipd.*', 'ipst.last_launch', 'ipst.last_status', 'ipst.last_result'],
        array $sorting = []
    ) {
        if ($join === null) {
            $join = [
                ...
                [
                    'table'     => ['?:import_preset_statistics' => 'ipst'],
    ...
    {
        $preset_data = array_merge([
            'company_id' => $this->company_id,
        ], $preset_data);
        $preset_data = $this->encodeResult($preset_data);
        return (bool) $this->db->replaceInto('import_preset_statistics', $preset_data);
    }
    ...
    public function delete($id)
    {
        ...
        $this->db->query('DELETE FROM ?:import_preset_statistics WHERE preset_id = ?i', $preset['preset_id']);
