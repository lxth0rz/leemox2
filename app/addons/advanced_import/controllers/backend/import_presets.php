...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if ($mode === 'upload') {
        ...
        if ($current_company && $current_company !== $preset['company_id'] && $preset['file_extension'] !== $old_preset['file_extension']) {
            fn_set_notification(NotificationSeverity::ERROR, __('error'), __('error_exim_cant_read_file'));
            return [CONTROLLER_STATUS_NO_CONTENT];
        }

        if ($current_company === 0 && (int) $current_company !== (int) $preset['company_id']) {
            $company_id = $preset['company_id'];
        } else {
            $company_id = $current_company;
        }

        $preset['file'] = $file_manager->moveUpload($file['name'], $file['path'], $company_id);
        if (isset(Tygh::$app['session']['import_file']['file_name'])) {
            $file_manager->removeFile(Tygh::$app['session']['import_file']['file_name'], $current_company);
        }
        Tygh::$app['session']['import_file']['file_name'] = $preset['file'];
        Tygh::$app['session']['import_file']['preset_id'] = $preset['preset_id'];
        if ($action === 'detailed') {
    ...
    if ($mode === 'update') {
        ...
        if ($current_company === 0 && (int) $current_company !== (int) $preset['company_id']) {
            $company_id = (int) $preset['company_id'];
        } else {
    ...
    if ($mode === 'remove_upload') {
        if (isset(Tygh::$app['session']['import_file']['file_name'])) {
            $file_manager->removeFile(Tygh::$app['session']['import_file']['file_name'], $current_company);
            unset(Tygh::$app['session']['import_file']);
        }
...
if ($mode === 'manage') {
    $params = array_merge([
        ...
        'object_type'       => 'products',
        'preview_preset_id' => isset(Tygh::$app['session']['import_file']['preset_id']) ? Tygh::$app['session']['import_file']['preset_id'] : 0,
    ], $_REQUEST);

    if (fn_allowed_for('MULTIVENDOR')) {
        $params['company_id'] = 0;
        list($common_presets, $search) = fn_get_import_presets($params);
        $common_presets = array_map(static function ($common_preset) use ($file_manager) {
            if ($common_preset['file_type'] === 'server') {
                $common_preset['file_path'] = $file_manager->getFilePath($common_preset['file'], $common_preset['company_id']);
            }
...
if ($mode === 'update') {
    ...
    if ($current_company && $preset['company_id'] !== $current_company && !isset($_REQUEST['start_import'])) {
        ...
        Registry::set('navigation.tabs.fields.href', 'import_presets.get_fields.get_mapping?preset_id=' . $_REQUEST['preset_id']);
...
if ($mode === 'get_fields') {
    ...
    if ($preset['preset_id']) {
        ...
        if (
            $current_company
            && $current_company !== $preset['company_id']
            && isset(Tygh::$app['session']['import_file']['file_name'])
        ) {
            $preset['file'] = Tygh::$app['session']['import_file']['file_name'];
        }
...
if ($mode === 'get_file') {
    ...
    $file_path = $file_manager->getFilePath($preset['file'], $preset['company_id']);
