...
class DiscussionsHookHandler
{
    ...
    /**
     * The "discussion_update_discussion_post" hook handler.
     *
     * Actions performed:
     *     - Sync discussion data per child variations
     *
     * @param array<string, int|string> $data Discussion data
     *
     * @see fn_update_discussion
     */
    public function onDiscussionUpdateDiscussionPost($data)
    {
        if ($data['object_type'] !== DiscussionObjectTypes::PRODUCT || empty($data['object_id'])) {
            return;
        }
        $sync_service = ServiceProvider::getSyncService();
        $sync_service->onTableChanged('discussion', (int) $data['object_id']);
    }
}
