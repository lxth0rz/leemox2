...
class ProductsHookHandler
{
    ...
    public function onUpdateImagePairsPre(
        ...
        foreach ($pairs_data as $pair_id => $pair_data) {
            ...
            if (empty($product_id) || !ServiceProvider::getProductIdMap()->isVariationProduct((int) $product_id)) {
                continue;
            }
            ...
            if ($pair_data['type'] !== ImagePairTypes::MAIN) {
                continue;
            }
            ...
            $key = isset($detailed[$product_id]) ? $product_id : 0;

            if (!isset($detailed[$key]['pairs_to_remove'])) {
                $detailed[$key]['pairs_to_remove'] = [];
            }

            /** @var array<int> $detailed[$key]['pairs_to_remove'] */
            $detailed[$key]['pairs_to_remove'][] = (int) $main_image['pair_id'];
