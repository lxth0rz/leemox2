...
function fn_vendor_debt_payout_is_grace_period_need_start($vendor_id)
{
    ...
    if ($balance < fn_vendor_debt_payout_get_lowers_allowed_balance($vendor_plan)) {
        return true;
    }
...
function fn_vendor_debt_payout_is_vendor_need_to_suspend($vendor_id)
{
    ...
    if ($grace_period >= (fn_vendor_debt_payout_get_grace_period_to_refill_balance($plan) * SECONDS_IN_DAY)) {
        return true;
    }
...
function fn_vendor_debt_payout_start_suspend_period($vendor_id)
{
    ...
    $reason_data = [
        ...
        '[date]'   => $formatter->asDatetime(fn_vendor_debt_payout_get_date_of_block($vendor_id, VendorStatuses::SUSPENDED)),
    ];
...
function fn_vendor_debt_payout_disable_vendor($vendor_id)
{
    ...
    $reason_data = [
        '[amount]' => $formatter->asPrice(fn_vendor_debt_payout_get_amount_to_pay($vendor_id)),
        '[date]'   => $formatter->asDatetime(fn_vendor_debt_payout_get_date_of_block($vendor_id, VendorStatuses::DISABLED)),
    ];
...
function fn_vendor_debt_payout_send_email_to_vendors_in_grace_period($vendor_id = 0)
{
    ...
    foreach ($vendors_ids_list as $vendor_id => $vendor_data) {
        ...
        $middle_of_grace_period = $vendor_data['grace_period_start'] + (fn_vendor_debt_payout_get_grace_period_to_refill_balance($vendor_plan) / 2) * SECONDS_IN_DAY;
...
function fn_vendor_debt_payout_dispatch_before_send_response($status, $area, $controller, $mode)
{
    ...
    if ($vendor_debt_data['debt_status'] === VendorDebtStatuses::IN_GRACE_PERIOD) {
        $message_data['[date]'] = $formatter->asDatetime(
            fn_vendor_debt_payout_get_date_of_block($vendor_id, VendorStatuses::SUSPENDED)
        );
 ...
 *
 * @param VendorPlan $plan Information about plan
 *
