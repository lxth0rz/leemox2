<tr class="import-preset" id="preset_{$preset.preset_id}">
    ...
    <td class="import-preset__file" data-th="{__("advanced_import.file")}">
        ...
        {elseif $preset.file_type == "Addons\\AdvancedImport\\PresetFileTypes::SERVER"|enum}
            ...
            {/if}
        {elseif $smarty.session.import_file && $smarty.session.import_file.preset_id == $preset.preset_id}
            ...
            {$smarty.session.import_file.file_name}
        {elseif "MULTIVENDOR"|fn_allowed_for && $preset.file && $runtime.company_id}
            {__("nothing_sign")}
        {elseif $preset.file}
            {btn type="dialog"
                text=__("choose")
    ...
    <td class="import-preset__run">
        ...
        || ($preset.file_type === "Addons\\AdvancedImport\\PresetFileTypes::LOCAL"|enum && $smarty.session.import_file && $smarty.session.import_file.preset_id == $preset.preset_id)
        }
