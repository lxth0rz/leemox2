            ...
            <tr class="hidden cm-row-status-{$a.status|lower} {$additional_class} cm-row-item {$addon_classes}" id="addon_{$key}{$suffix}" data-supplier="{$a.supplier}">
                ...
                <td width="80%">
                    ...
                    {if $is_editable}
                        ...
                        {else}
                            <a class="row-status cm-external-click {if !$is_editable}no-underline{/if} {if !$a.snapshot_correct}cm-promo-popup{/if}" {if $a.snapshot_correct}data-ca-external-click-id="opener_group{$key}"{/if}>{$a.name|default:$key}</a>
                        {/if}
                    {else}
                ...
                <td width="15%" class="addon-action">
                    ...
                    {else}
                        <div class="nowrap">
                            ...
                            status_target_id="addons_list,header_navbar,header_subnav,addons_counter,elm_developer_pages,elm_all_dev_pages"
